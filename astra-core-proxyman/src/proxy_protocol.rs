...
pub async fn accept_proxy_protocol(conn: &mut tokio::net::TcpStream) -> Result<Option<String>, String> {
    // PROXY protocol v1: "PROXY TCP4/AF_UNSPEC src_ip dst_ip src_port dst_port\r\n"
    let mut buf = [0u8; 108]; // Max PROXY v1 header size
    let n = conn.peek(&mut buf).await.map_err(|e| format!("proxy peek: {}", e))?;
    ...
    let header_end = buf[..n].windows(2).position(|w| w == b"\r\n")
        .ok_or_else(|| "proxy protocol: no CRLF found".to_string())? + 2;
    ...
    conn.read_exact(&mut temp).await.map_err(|e| format!("proxy read: {}", e))?;
