...
impl CipherType {
    ...
    pub fn nonce_size(&self) -> usize {
        match self { CipherType::XChaCha20Poly1305 => 24, _ => 12 }
    }
...
fn aead_encrypt(c: CipherType, key: &[u8], nonce: &[u8], data: &[u8]) -> Result<Vec<u8>, String> {
    match c {
        CipherType::Aes128Gcm => {
            ...
            cipher.encrypt(n, data).map_err(|e| format!("aes encrypt: {}", e))
        }
        ...
        CipherType::XChaCha20Poly1305 => {
            ...
            cipher.encrypt(n, data).map_err(|e| format!("xchacha encrypt: {}", e))
        }
    }
}
...
fn aead_decrypt(c: CipherType, key: &[u8], nonce: &[u8], data: &[u8]) -> Result<Vec<u8>, String> {
    match c {
        CipherType::Aes128Gcm => {
            let cipher = Aes128Gcm::new_from_slice(key).map_err(|e| format!("key: {}", e))?;
            let n = aes_gcm::Nonce::from_slice(nonce);
            cipher.decrypt(n, data).map_err(|e| format!("aes decrypt: {}", e))
        }
        CipherType::Aes256Gcm => {
            let cipher = Aes256Gcm::new_from_slice(key).map_err(|e| format!("key: {}", e))?;
            let n = aes_gcm::Nonce::from_slice(nonce);
        ...
        CipherType::XChaCha20Poly1305 => {
            let cipher = chacha20poly1305::XChaCha20Poly1305::new_from_slice(key)
                .map_err(|e| format!("key: {}", e))?;
            let n = chacha20poly1305::XNonce::from_slice(nonce);
            cipher.decrypt(n, data).map_err(|e| format!("xchacha decrypt: {}", e))
        }
    }
}

fn inc_nonce(n: &mut [u8]) {
    for b in n.iter_mut().rev() { *b = b.wrapping_add(1); if *b != 0 { break; } }
}

// ─── TCP AEAD chunked (Go: shadowaead TCP) ─────────────────────────────────

pub async fn write_chunk<W: tokio::io::AsyncWrite + Unpin>(
    w: &mut W, data: &[u8], c: CipherType, key: &[u8], nonce: &mut [u8],
) -> Result<(), String> {
...
pub async fn read_chunk<R: tokio::io::AsyncRead + Unpin>(
    r: &mut R, c: CipherType, key: &[u8], nonce: &mut [u8],
) -> Result<Option<Vec<u8>>, String> {
    use tokio::io::AsyncReadExt;
    let mut len_enc = vec![0u8; 2 + TAG];
    if r.read_exact(&mut len_enc).await.is_err() { return Ok(None); }

    let len_pt = aead_decrypt(c, key, nonce, &len_enc)?;
    inc_nonce(nonce);

    let payload_len = u16::from_be_bytes([len_pt[0], len_pt[1]]) as usize;
    if payload_len == 0 || payload_len > MAX_PKT { return Err("bad len".into()); }

    let mut payload_enc = vec![0u8; payload_len + TAG];
    r.read_exact(&mut payload_enc).await.map_err(|e| e.to_string())?;
...
pub fn encrypt_udp(
    c: CipherType, key: &[u8], session_id: &[u8], pkt_id: &[u8], data: &[u8],
) -> Result<Vec<u8>, String> {
    match c {
        CipherType::XChaCha20Poly1305 => {
            let mut nonce = [0u8; 24];
            for b in &mut nonce { *b = rand::random(); }
...
pub fn decrypt_udp(
    c: CipherType, key: &[u8], data: &[u8],
) -> Result<(Vec<u8>, Vec<u8>), String> {
    match c {
        CipherType::XChaCha20Poly1305 => {
            if data.len() < 24 + TAG { return Err("short".into()); }
            let pt = aead_decrypt(c, key, &data[..24], &data[24..])?;
            Ok((vec![], pt))
        }
        _ => {
            if data.len() < 16 + TAG { return Err("short".into()); }
            let mut header = [0u8; 16];
...
pub fn encode_target(addr: &astra_core_net::Address, port: u16) -> Vec<u8> {
    ...
    match addr {
        ...
        astra_core_net::Address::Domain(d) => {
            out.push(2); out.push(d.len() as u8); out.extend_from_slice(d.as_bytes());
        }
...
pub fn decode_target(data: &[u8]) -> Result<(astra_core_net::Address, u16), String> {
    ...
    match data[0] {
        1 => {
            ...
            Ok((astra_core_net::Address::Ipv4([data[1], data[2], data[3], data[4]]),
        ...
        2 => {
            ...
            Ok((astra_core_net::Address::Domain(domain),
        ...
        3 => {
            ...
            Ok((astra_core_net::Address::Ipv6(ip),
...
mod tests {
    ...
    async fn test_tcp_chunk_roundtrip() {
        ...
        write_chunk(&mut tx, b"hello ss2022!", CipherType::Aes128Gcm, &key, &mut wn).await.unwrap();
        let res = read_chunk(&mut rx, CipherType::Aes128Gcm, &key, &mut rn).await.unwrap();
    ...
    async fn test_tcp_chunk_roundtrip_xchacha() {
        ...
        write_chunk(&mut tx, b"xchacha test", CipherType::XChaCha20Poly1305, &key, &mut wn).await.unwrap();
        let res = read_chunk(&mut rx, CipherType::XChaCha20Poly1305, &key, &mut rn).await.unwrap();
