...
impl RequestConfig {
    ...
    fn pick_headers(&self) -> Vec<String> {
        self.header
            .iter()
            .map(|h| format!("{}: {}", h.name, pick_str(&h.value)))
            .collect()
    }
...
impl ResponseConfig {
    fn has_header(&self, name: &str) -> bool {
        ...
        self.header
            .iter()
            .any(|h| h.name.to_ascii_lowercase() == lower)
    }
    fn pick_headers(&self) -> Vec<String> {
        self.header
            .iter()
    ...
    fn get_status(&self) -> Status {
        self.status.clone().unwrap_or(Status {
            code: "200".into(),
            reason: "OK".into(),
        })
    }
    ...
    loop {
        ...
        if let Some(pos) = buf.windows(4).position(|w| w == b"\r\n\r\n") {
            ...
            if let Some(expected) = expected {
                let header_str =
                    std::str::from_utf8(header_bytes).map_err(|_| "header: invalid utf8")?;
    ...
    fn poll_read(
        ...
        if let Some(ref data) = self.read_pending {
            if !data.is_empty() {
                ...
                self.read_pending = if remaining.is_empty() {
                    None
                } else {
                    Some(remaining)
                };
    ...
    pub async fn client_handshake<T: AsyncRead + AsyncWrite + Unpin>(
        ...
        if let Some(ref req) = self.config.request {
            ...
            hc.inner
                .write_all(&header)
                .await
                .map_err(|e| format!("client header: {}", e))?;
        }
        Ok(hc)
    }
    ...
    pub async fn server_handshake<T: AsyncRead + AsyncWrite + Unpin>(
        ...
        if let Some(ref resp) = self.config.response {
            ...
            hc.inner
                .write_all(&header)
                .await
                .map_err(|e| format!("server header: {}", e))?;
        }
        Ok(hc)
    }
}
...
mod tests {
    ...
    async fn test_read_header_mismatch() {
        ...
        let mut cur = std::io::Cursor::new(data.as_slice());
        let expected = RequestConfig {
            uri: vec!["/expected".into()],
            ..Default::default()
        };
    ...
    async fn test_authenticator_client_handshake() {
        ...
        let n = tokio::time::timeout(std::time::Duration::from_secs(1), server.read(&mut buf))
            .await
            .unwrap()
            .unwrap();
